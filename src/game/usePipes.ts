...
import { GROUND_H, PIPE_W, SPAWN_EVERY, VIEW_H, VIEW_W } from "./constants";
...
export function usePipes() {
  ...
  const update = useCallback((dt: number, speed: number, gapH: number, onPass?: (p: Pipe) => void) => {
    setPipes(prev => {
      const next = prev.map(p => ({ ...p, x: p.x - speed * dt }));
